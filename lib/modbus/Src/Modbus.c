...
void vTimerCallbackT35(TimerHandle_t *pxTimer)
{
	...
	for(i = 0; i < numberHandlers; i++)
	{
		...
		if( (TimerHandle_t *)mHandlers[i]->xTimerT35 ==  pxTimer ){
            //printf("T35: handler found, notifying\r\n");
...
void StartTaskModbusSlave(void *argument)
{
  ...
  for(;;)
  {
   ...
   //printf("MDB: wait notify\r\n");
   ulTaskNotifyTake(pdTRUE, portMAX_DELAY); /* Block until a Modbus Frame arrives */
   //printf("MDB: got notify\r\n");

   if (getRxBuffer(modH) == ERR_BUFF_OVERFLOW)
   {
		modH->i8lastError = ERR_BUFF_OVERFLOW;
    	modH->u16errCnt++;
        printf("MDB: ERR_BUFF_OVERFLOW\r\n");
  continue;
}
   ...
   if (modH->u8BufferSize < 7)
   {
		...
		modH->u16errCnt++;
		printf("MDB: ERR_BAD_SIZE: %d\r\n", modH->u8BufferSize);
		// debug_print_buffer(modH->u8Buffer, modH->u8BufferSize);
		continue;
		  }
   ...
   // check slave id
    if ( modH->u8Buffer[ID] !=  modH->u8id && modH->u8AddressMode != ADDRESS_BROADCAST)
	{
	 ...
	 // debug_uart_send_string("MDB: validation\r\n");
	...
	if (u8exception > 0){
		...
		printf("MDB: validation failed %u\r\n", u8exception);
		//return u8exception
...
*/
/*
 * This busy-waiting loop is a critical design flaw in an RTOS environment.
 * It blocks the entire CPU, preventing other tasks from running.
 * The HAL_UART_Transmit_IT function is non-blocking, and we should rely on
 * RTOS primitives (like notifications or semaphores) to wait for completion,
 * not a while loop. Commenting this out should fix the system freeze under high load.
 */
